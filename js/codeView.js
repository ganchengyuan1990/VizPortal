    ...
    var deleteDefaultJSData=['sap-ui-bootstrap','beautify', 'mode','codemirror','codeView','json2','rgbcolor_static'];
        ...
        for(var i=0;i<lines.length;i++)
        {
           ...
           if(tempStr.indexOf("sap.viz.api.env.Template.loadPaths")!=-1)
           {
             ...
             for(var i=0;i<tempArry.length;i++)
             {
                if(i==0)
                {
                  replaceData=replaceData+tempArry[i]+","+"\n" ;
                }
                else
                {
                  replaceData=replaceData+tempArry[i]+"\n";
